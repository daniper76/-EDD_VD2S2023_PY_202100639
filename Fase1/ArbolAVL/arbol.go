...
func (a *ArbolAVL) InsertarElemento(valor string, nombre string) {
	nuevoNodo := &NodoArbol{Valor: valor, Nombre: nombre}
...
func (a *ArbolAVL) LeerJson(ruta string) {
	...
	for _, curso := range datos.Cursos {
		a.InsertarElemento(curso.Codigo, curso.Nombre)
	}
}
